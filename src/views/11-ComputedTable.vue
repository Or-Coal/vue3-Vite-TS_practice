        ...
        <div class="card-body">
            <table class="table">
                <thead>
                    <tr>
                        <th>
                            <input v-model="allChecked" type="checkbox" class="from-check-input">
                        </th>
                        <th scope="col">#</th>
                        <th scope="col">书名</th>
                        <th scope="col">作者</th>
                ...
                <tfoot>
                    <tr>
                        <td colspan="6">
                            总计<span class="text-danger">{{ total_nums }}</span>本书，
                            总价格为<b class="text-danger">{{ total_price }}</b>元
                        </td>
                    </tr>
                ...
                <tbody>
                    <tr v-for="(item, index) in books" :key="item.id">
                        <td>
                            <input v-model="item.checked" type="checkbox" class="from-check-input">
                        </td>
                        <td>{{ item.id }}</td>
                        <td>{{ item.name }}</td>
                        <td>{{ item.author }}</td>
...
const books = reactive([
    ...
    { id: 2, author: '施耐庵', name: '水浒传', num: 3, price: 30.0, checked: true },
    { id: 3, author: '罗贯中', name: '三国演义', num: 2, price: 24.0, checked: false },
...
const allChecked = computed({
    ...
    set(newVlaue) {
        books.forEach((book) => { book.checked = newVlaue });
    }
})
const total_nums = computed(() => {
    return books.reduce((total, book) => total +(book.checked?book.num:0), 0)
})
const total_price = computed(() => {
    let total = 0
    books.forEach(item => {if(item.checked){total+=item.price * item.num}})
    return total
})
function handleSort(){
